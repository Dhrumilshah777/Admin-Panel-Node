    ...
    try{
        ...
        if(checkEmailId){
            const transporter = nodemailer.createTransport({
                ...
                auth: {
                    user: "dhrumilshah71011@gmail.com",
                    pass: "fkomxibwpuxfgvms",
                },
            });
            ...
            const info = await transporter.sendMail({
                from: '<dhrumilshah71011@gmail.com>',
                to: req.body.email,
...
module.exports.updateAdmin = async (req, res) => {
    try {
        const editAdminId = req.query.adminId;

        if (!editAdminId) return res.redirect('back');

        const oldAdminData = await Admin.findById(editAdminId);
        if (!oldAdminData) return res.redirect('back');
...
module.exports.editAdminData = async (req, res) => {
    try {
        ...
        const oldAdminData = await Admin.findById(adminId);
        ...
        let updateData = {
            ...
            desc: req.body.desc,
            avtar: oldAdminData.avtar
        };

        if (req.file) {
            if (oldAdminData.avtar) {
                const oldPath = path.join(__dirname, "..", oldAdminData.avtar);
                if (fs.existsSync(oldPath)) fs.unlinkSync(oldPath);
            }
            updateData.avtar = Admin.adPath + "/" + req.file.filename;
        }
